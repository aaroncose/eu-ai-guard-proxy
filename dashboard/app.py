...
with st.sidebar:
    ...
    st.markdown("---")
    allow_reset = settings.DEBUG or os.getenv("ALLOW_DEMO_RESET", "false").lower() == "true"
    if allow_reset:
        st.markdown("**Zona de Pruebas (Desarrollo):**")
        confirm_reset = st.checkbox("Habilitar purga local")
        if confirm_reset and st.button("Vaciar Registros Locales", use_container_width=True, type="secondary"):
            asyncio.run(reset_demo_database())
            st.info("Base de datos reiniciada.")
            st.rerun()
    else:
        st.caption("🔒 Purga inhabilitada: Inmutabilidad activa (Art. 12)")
